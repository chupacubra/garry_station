    ...
    timer.Simple(0.2, function()
        ...
        if IsValid(ent) then
            if ent:IsPlayer() then
                player_manager.RunClass( ent, "HurtPart", trace.PhysicsBone, {[D_BRUTE] = math.random(3, 5)})
                ent:SetVelocity( trace.Normal * punch_force )
            else
